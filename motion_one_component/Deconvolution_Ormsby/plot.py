...
surface_acc = np.loadtxt('acc_ormsby.txt'); 

acc_depth12 = np.loadtxt('z_at_depth_12_acc.txt');   
...
ax.plot(surface_acc[:, 0], surface_acc[:, 1], '-r', label='input motion @ surface', linewidth= 2.0); 

ax.plot(acc_depth12[:, 0], acc_depth12[:, 1], '-k', label='deconvolution motion @ 12m depth', linewidth= 1.0); 
...
plt.savefig('acc_deconvolution.pdf', bbox_inches='tight'); 
