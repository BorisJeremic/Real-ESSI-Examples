...
input_acc = np.loadtxt('surface_acc_analytical.txt'); 

input_disp = np.loadtxt('surface_disp_analytical.txt'); 
...
ax.plot(input_acc[:, 0], input_acc[:, 1], '-r', label='surface analytical', linewidth= 1.5); 

ax.plot(Time, acceleration_component_surface[0, :], '-k', label='DRM propagation', linewidth= 0.5); 
